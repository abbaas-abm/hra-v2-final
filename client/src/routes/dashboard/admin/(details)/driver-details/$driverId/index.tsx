...
import { FaBuilding, FaCalendar, FaCar, FaEnvelope, FaKey, FaPhone, FaUser } from 'react-icons/fa';
...
export const Route = createFileRoute(
  ...
  head: () => ({
   ...
   loader: async ({context: {queryClient}, params}) => {
    return queryClient.ensureQueryData(profileQueryOptions(params.driverId))
  }
})
...
function DriverDetailsPage() {
  const {driverId} = Route.useParams();
    ...
    const {data:driver} = useSuspenseQuery(profileQueryOptions(driverId))
  const {
    companyDetails: {
      companyName,
      regIdNumber,
    ...
    <div className="min-h-screen bg-white p-4 sm:p-6 lg:p-8 w-full">
      <div className="max-w-7xl mx-auto">
        <h1 className="text-3xl font-bold text-black mb-8 text-center flex items-center justify-center">
          <FaUser className="mr-2 text-yellow-500" /> Driver Details
        </h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 border-l-4 border-yellow-400 hover:shadow-xl transition-shadow">
            <h2 className="text-xl font-semibold text-black mb-4 flex items-center">
              <FaUser className="mr-2 text-yellow-500" /> Personal Information
            </h2>
            ...
            <p className="text-black mb-2 flex items-center">
              <FaEnvelope className="mr-2 text-yellow-500" /> <span className="font-medium">Email:</span> {email}
            </p>
            <p className="text-black mb-2 flex items-center">
              <FaPhone className="mr-2 text-yellow-500" /> <span className="font-medium">Mobile:</span> {mobileNumber}
            </p>
          </div>
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 border-l-4 border-yellow-400 hover:shadow-xl transition-shadow">
            <h2 className="text-xl font-semibold text-black mb-4 flex items-center">
              <FaBuilding className="mr-2 text-yellow-500" /> Company Information
            </h2>
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 border-l-4 border-yellow-400 hover:shadow-xl transition-shadow">
            <h2 className="text-xl font-semibold text-black mb-4 flex items-center">
              <FaCar className="mr-2 text-yellow-500" /> Vehicle Information
            </h2>
          ...
          <div className="bg-white shadow-lg rounded-lg p-6 border-l-4 border-yellow-400 hover:shadow-xl transition-shadow sm:col-span-2 lg:col-span-3">
            <h2 className="text-xl font-semibold text-black mb-4 flex items-center">
              <FaKey className="mr-2 text-yellow-500" /> Account Details
            </h2>
            <p className="text-black mb-2"><span className="font-medium">Driver ID:</span> {_id}</p>
